...
const resetearCarta = (cartareseteo) => {
  ...
  cartareseteo.nodoHTML.style.backgroundImage =
...
const comprobar = (carta1, carta2) => {
  ...
  if (carta1.color == carta2.color) {
    ...
    carta1.nodoHTML.classList.add('emparejada');
    carta2.nodoHTML.classList.add('emparejada');
    contador = 0;
...
const mostrarVictoria = () => {
  ...
  victoriaMensaje.textContent = '¡Victoria! Has encontrado todas las parejas';

  document.querySelector('#Memoria').appendChild(victoriaMensaje);
};
...
export const Cards = (number = 10) => {
  ...
  for (let i = 0; i < totalCards; i++) {
    ...
    cardDiv.addEventListener('click', () =>
      seleccionar(cardDiv.firstChild, card.color, card.id)
    );
    cartas.append(card);
  }
};
